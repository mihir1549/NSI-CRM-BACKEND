...
export class BroadcastAdminController {
   ...
   * Create an announcement or broadcast message as SUPER_ADMIN.
   */
  @ApiOperation({ summary: 'Admin: create a broadcast or announcement' })
  @ApiResponse({
    status: 201,
  ...
  async create(@Body() dto: CreateBroadcastDto, @Req() req: Request) {
    ...
    return this.broadcastService.createAdminBroadcast(dto, user.sub);
  }

  /**
   * GET /api/v1/admin/broadcasts
   * Paginated list of all broadcasts and announcements.
   */
  @ApiOperation({ summary: 'Admin: list all broadcasts with pagination' })
  @ApiQuery({
    name: 'page',
    required: false,
    example: 1,
  ...
  async list(@Query('page') page = '1', @Query('limit') limit = '20') {
    return this.broadcastService.getAdminBroadcasts(
   ...
   * Deactivate any broadcast or announcement.
   */
  @ApiOperation({ summary: 'Admin: deactivate a broadcast or announcement' })
