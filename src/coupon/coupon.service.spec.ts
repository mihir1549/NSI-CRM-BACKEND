...
describe('CouponService', () => {
  ...
  describe('validateCoupon', () => {
    ...
    it('throws NotFoundException if coupon missing', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue(null);
      await expect(service.validateCoupon('INVALID', 'user1', PaymentType.LMS_COURSE, 1000)).rejects.toThrow(NotFoundException);
    });

    it('throws BadRequestException if inactive', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ ...validCoupon, isActive: false });
      await expect(service.validateCoupon('SAVE10', 'user1', PaymentType.LMS_COURSE, 1000)).rejects.toThrow(BadRequestException);
    });

    it('throws BadRequestException if expired', async () => {
      const pastDate = new Date();
      pastDate.setFullYear(2020);
      mockPrisma.coupon.findUnique.mockResolvedValue({ ...validCoupon, expiresAt: pastDate });
      await expect(service.validateCoupon('SAVE10', 'user1', PaymentType.LMS_COURSE, 1000)).rejects.toThrow(BadRequestException);
    });

    it('throws BadRequestException if global usage limit reached', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ ...validCoupon, usageLimit: 5, usedCount: 5 });
      await expect(service.validateCoupon('SAVE10', 'user1', PaymentType.LMS_COURSE, 1000)).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws BadRequestException if scope mismatch', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ ...validCoupon, applicableTo: CouponScope.DISTRIBUTOR_SUB });
      await expect(service.validateCoupon('SAVE10', 'user1', PaymentType.LMS_COURSE, 1000)).rejects.toThrow(BadRequestException);
    });

    it('returns discount amount for Flat', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue(validCoupon);
      const result = await service.validateCoupon('SAVE10', 'user1', PaymentType.LMS_COURSE, 1000);
      expect(result.discountAmount).toBe(100);
      expect(result.finalAmount).toBe(900);
    });

    it('returns discount amount for Percent', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ ...validCoupon, type: CouponType.PERCENT, value: 15 });
      const result = await service.validateCoupon('SAVE10', 'user1', PaymentType.LMS_COURSE, 1000);
      expect(result.discountAmount).toBe(150);
    });

    it('returns free for Free type', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ ...validCoupon, type: CouponType.FREE });
      const result = await service.validateCoupon('SAVE10', 'user1', PaymentType.LMS_COURSE, 1000);
  ...
  describe('validateCouponInTx', () => {
    ...
    it('works correctly inside a transaction', async () => {
      mockTx.coupon.findUnique.mockResolvedValue({
        uuid: 'c1', isActive: true, expiresAt: null, usageLimit: null, usedCount: 0, perUserLimit: 1, applicableTo: CouponScope.ALL, type: CouponType.FLAT, value: 50
      });
      mockTx.couponUse.count.mockResolvedValue(0);

      const result = await service.validateCouponInTx('TXCODE', 'u1', PaymentType.LMS_COURSE, 100, mockTx);
  ...
  describe('createCoupon', () => {
    it('creates a valid coupon', async () => {
      const dto = { code: ' NEWCODE ', type: CouponType.FLAT, value: 10, applicableTo: CouponScope.ALL, perUserLimit: 1 };
      mockPrisma.coupon.findUnique.mockResolvedValue(null);
      mockPrisma.coupon.create.mockResolvedValue({ ...dto, code: 'NEWCODE', isActive: true });

      const result = await service.createCoupon(dto);
      expect(mockPrisma.coupon.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ code: 'NEWCODE' }) }));
      expect(result.status).toBe('ACTIVE');
    });

    it('throws ConflictException if code exists', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ id: 1 });
      await expect(service.createCoupon({ code: 'EXISTS', type: CouponType.FLAT, value: 10, applicableTo: CouponScope.ALL, perUserLimit: 1 })).rejects.toThrow(ConflictException);
    });
  ...
  describe('getCouponDetail', () => {
    it('returns detail', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ isActive: true, expiresAt: null });
    ...
    it('throws Not Found', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue(null);
      await expect(service.getCouponDetail('uuid')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getCouponForUpdate', () => {
    it('returns updatable fields', async () => {
      const date = new Date();
      mockPrisma.coupon.findUnique.mockResolvedValue({ isActive: false, expiresAt: date, usageLimit: 10 });
    ...
    it('throws Not Found', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue(null);
      await expect(service.getCouponForUpdate('uuid')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('updateCoupon', () => {
    it('updates correctly', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue({ isActive: true, expiresAt: null });
      mockPrisma.coupon.update.mockResolvedValue({ isActive: false, expiresAt: null });
    ...
    it('throws BadRequestException for reactivating expired coupon', async () => {
      ...
      mockPrisma.coupon.findUnique.mockResolvedValue({ isActive: false, expiresAt: past });

      await expect(service.updateCoupon('uuid', { isActive: true })).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws Not Found', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue(null);
      await expect(service.updateCoupon('uuid', { isActive: true })).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('deleteCoupon', () => {
    it('soft deletes if used', async () => {
      ...
      expect(mockPrisma.coupon.update).toHaveBeenCalledWith(expect.objectContaining({ data: { isActive: false }}));
    ...
    it('throws Not Found', async () => {
      mockPrisma.coupon.findUnique.mockResolvedValue(null);
      await expect(service.deleteCoupon('uuid')).rejects.toThrow(NotFoundException);
    });
  });
