...
const mockPrisma = {
  ...
  funnelProgress: { findUnique: jest.fn(), create: jest.fn(), update: jest.fn() },
...
describe('PhoneService', () => {
  ...
  describe('sendOtp', () => {
    it('sends OTP successfully', async () => {
      const result = await service.sendOtp(USER_UUID, '09999999999', 'sms', '127.0.0.1');

      expect(result.message).toBe('OTP sent successfully');
      expect(mockPhoneProvider.sendOtp).toHaveBeenCalledWith('+919999999999', 'sms');
    ...
    it('throws HttpException if rate limit exceeded', async () => {
      ...
      await expect(service.sendOtp(USER_UUID, MOCK_PHONE, 'sms', '127.0.0.1')).rejects.toThrow(HttpException);
    ...
    it('throws ConflictException if phone registered to another user', async () => {
      mockPrisma.userProfile.findUnique.mockResolvedValue({ userUuid: OTHER_USER_UUID, phone: MOCK_PHONE });

      await expect(service.sendOtp(USER_UUID, MOCK_PHONE, 'sms', '127.0.0.1')).rejects.toThrow(ConflictException);
      expect(mockAudit.log).toHaveBeenCalledWith(expect.objectContaining({ action: 'PHONE_ALREADY_REGISTERED_ATTEMPT' }));
    });

    it('throws ConflictException if user already has phoneVerified', async () => {
      mockPrisma.userProfile.findUnique.mockResolvedValue(null);
      mockPrisma.funnelProgress.findUnique.mockResolvedValue({ phoneVerified: true });
  ...
  describe('verifyOtp', () => {
    it('verifies OTP successfully and updates funnel progress', async () => {
      const result = await service.verifyOtp(USER_UUID, MOCK_PHONE, '123456', 'sms', '127.0.0.1');

      expect(result.message).toBe('Phone verified successfully');
      expect(mockPhoneProvider.verifyOtp).toHaveBeenCalledWith(MOCK_PHONE, '123456', 'sms');
      expect(mockPrisma.userProfile.upsert).toHaveBeenCalled();
      expect(mockPrisma.funnelProgress.create).toHaveBeenCalledWith(expect.objectContaining({
        data: expect.objectContaining({ userUuid: USER_UUID, phoneVerified: true })
      }));
      expect(mockAudit.log).toHaveBeenCalledWith(expect.objectContaining({ action: 'PHONE_VERIFIED' }));
    ...
    it('throws BadRequestException for invalid OTP and increments attempt tracker', async () => {
      ...
      await expect(service.verifyOtp(USER_UUID, MOCK_PHONE, '000000', 'sms', '127.0.0.1')).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws HttpException if max wrong attempts reached', async () => {
      ...
      // Attempt 1
      await expect(service.verifyOtp(USER_UUID, MOCK_PHONE, '000', 'sms', '127.0.0.1')).rejects.toThrow(BadRequestException);
      // Attempt 2
      await expect(service.verifyOtp(USER_UUID, MOCK_PHONE, '000', 'sms', '127.0.0.1')).rejects.toThrow(BadRequestException);
      // Attempt 3
      await expect(service.verifyOtp(USER_UUID, MOCK_PHONE, '000', 'sms', '127.0.0.1')).rejects.toThrow(BadRequestException);
      ...
      await expect(service.verifyOtp(USER_UUID, MOCK_PHONE, '000', 'sms', '127.0.0.1')).rejects.toThrow(HttpException);
    });
    ...
    it('advances funnel steps if progress already exists', async () => {
      ...
      mockPrisma.funnelStep.findUnique.mockResolvedValue({ uuid: 'step-1', sectionUuid: 'section-1', order: 1 });
      mockPrisma.funnelStep.findFirst.mockResolvedValue({ uuid: 'step-2', sectionUuid: 'section-1', order: 2 });
      mockPrisma.funnelProgress.update.mockResolvedValue({ phoneVerified: true, currentStepUuid: 'step-2' });
